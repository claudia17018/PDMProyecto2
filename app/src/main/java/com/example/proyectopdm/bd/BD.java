...
public class BD {
        ...
        public void onCreate(SQLiteDatabase db) {
            try {
                ...
                db.execSQL("create table RESUMENSERVICIOTOTAL \n" +
                        ...
                        "   FECHAEMISIONCERTIFICADO DATE                 not null,\n" +
    ...
    public void llenarDB(){
      ...
      /********** Area **************/
        ...
        final String[] dtDuiTu={"6565656659","6565656659"};
       ...
       /************* LLENADO DE BASE DE DATOS************************/
        ...
        if(selectUsuarios().isEmpty()) {
            ...
            ResumenServicio reSe = new ResumenServicio();
            for (int i = 0; i < 2; i++) {
                reSe.setCarnetEs(dtCarnet[i]);
                reSe.setDuiTu(dtDuiTu[i]);
                reSe.setFechaApertura(dtFechaA[i]);
                reSe.setFechaCertificado(dtFechaC[i]);
                insertarDetalleResumen(reSe);
            }

            DetalleServicio deSe = new DetalleServicio();
            for (int i = 0; i < 2; i++) {
                deSe.setIdRes(drIdRes[i]);
                deSe.setHorasAsignadas(drHa[i]);
                deSe.setDirectos(drBD[i]);
                deSe.setIndirectos(drBI[i]);
                deSe.setObservaciones(drObser[i]);
                deSe.setEstado(drEstado[i]);
    ...
    */
    ...
    public String insertarDetalleResumen(ResumenServicio rs){
        long contador=0;
        ContentValues alu = new ContentValues();

        alu.put("CARNET",rs.getCarnetEs());
        alu.put("DUITUTOR",rs.getDuiTu());
        alu.put("FECHAAPERTURAEXPEDIENTE",rs.getFechaApertura());
        alu.put("FECHAEMISIONCERTIFICADO",rs.getFechaCertificado());
    ...
    public String insertarDetalleServicio(DetalleServicio ds){
        long contador=0;
        ContentValues alu = new ContentValues();

        alu.put("IDRESUMEN",ds.getIdRes());
        alu.put("HORASASIGNADAS",ds.getHorasAsignadas());
        alu.put("BENEFICIARIOSDIRECTOS",ds.getDirectos());
        alu.put("BENEFICIARIOSINDIRECTOS",ds.getIndirectos());
        alu.put("OBSERVACIONES",ds.getObservaciones());
        alu.put("ESTADODERESUMEN",ds.getEstado());
