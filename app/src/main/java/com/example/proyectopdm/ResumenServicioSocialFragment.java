...
public class ResumenServicioSocialFragment extends Fragment {
    ...
    ArrayList<ResumenServicio> listadoResumenServicio;
    ...
    private ArrayList<ResumenServicio> consultarListadoResumenServicio() {
        sqLiteDatabase = dataBaseHelper.getReadableDatabase();

        Cursor cursor = sqLiteDatabase.rawQuery("SELECT * FROM RESUMENSERVICIOTOTAL",null);
        ArrayList<ResumenServicio> listadoResumenServicio = new ArrayList<>();

        if (cursor.moveToFirst()) {
            do {
                listadoResumenServicio.add(new ResumenServicio(cursor.getInt(0),cursor.getString(1), cursor.getString(2),cursor.getString(3),cursor.getString(4)));
