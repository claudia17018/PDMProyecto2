...
public class DetalleServicioFragment extends Fragment {
    ...
    ArrayList<DetalleServicio> listadoDetalleServicio;
    ...
    private ArrayList<DetalleServicio> consultarListadoDetalleServicio() {
        sqLiteDatabase = dataBaseHelper.getReadableDatabase();

        Cursor cursor = sqLiteDatabase.rawQuery("SELECT * FROM DETALLERESUMENSERVICIO",null);
        ArrayList<DetalleServicio> listadoDetalleServicio = new ArrayList<>();

        if (cursor.moveToFirst()) {
            do {
                listadoDetalleServicio.add(new DetalleServicio(cursor.getInt(0),cursor.getInt(1),cursor.getInt(2),cursor.getInt(3),cursor.getInt(4), cursor.getString(5),cursor.getString(6)));
